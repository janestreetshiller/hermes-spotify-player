...
await check('Settings and taste palette controls: persistence, curation, prompt selection and hover inventory',async p=>{
 ...
 await button(p,'Taste tracks').click();await shell(p).getByRole('textbox',{name:'Playlist tracks'}).fill('spotify:track:aaaaaaaaaaaaaaaaaaaaaa');await button(p,'Taste actions').click();await button(p,'Read taste').click();await p.getByText('Taste sample · 0 recent liked songs',{exact:true}).waitFor();await button(p,'Back to taste').click();await button(p,'Create private playlist').click();await p.getByText('Verified · 1 track saved',{exact:true}).waitFor();await button(p,'Back to taste').click();await button(p,'Like draft tracks').click();await p.getByText('Verified · 1 liked state updated',{exact:true}).waitFor();await button(p,'Back to taste').click();await button(p,'Remove draft likes').click();await p.waitForTimeout(250)
 await button(p,'Back to taste').click();await button(p,'LLM prompt').click();await button(p,'Copy prompt').waitFor();await button(p,'Next message').click();await button(p,'Back to taste').click()
 const inventory=[]
...
await check('Settings keeps text controls plus exactly one reference-layout picker and fully fits without scrolling',async p=>{
 await button(p,'Player settings').click();const settings=shell(p).getByLabel('Player settings panel');assert.equal(await settings.locator('button,svg').count(),0,'no decorative hardware buttons inside settings');
 assert.equal(await settings.locator('select').count(),1,'only the explicitly added layout picker is allowed');
 const picker=settings.getByRole('combobox',{name:'Player style',exact:true});assert.equal(await picker.count(),1);assert.deepEqual(await picker.locator('option').evaluateAll(options=>options.map(o=>o.value)),['silver','quicktime','wmp-curved','handheld','droplet','wmp-classic','wmp-glass','jetaudio','organic','dark-playlist','wmp-library']);
 for(const width of [234,280,340]){await p.locator('.product').evaluate((e,w)=>e.style.width=w+'px',width);await p.waitForTimeout(150);const fit=await settings.evaluate(e=>{const r=e.getBoundingClientRect(),surface=e.closest('.spotify-surface'),transform=new DOMMatrixReadOnly(getComputedStyle(surface).transform),scale=Math.hypot(transform.a,transform.b);return {fonts:[...e.querySelectorAll('label,select,header')].map(n=>parseFloat(getComputedStyle(n).fontSize)*scale),height:e.clientHeight,content:e.scrollHeight,width:e.clientWidth,contentWidth:e.scrollWidth,overflow:getComputedStyle(e).overflowY,children:[...e.querySelectorAll('label,a,header')].map(n=>{const b=n.getBoundingClientRect();return b.top>=r.top&&b.bottom<=r.bottom&&b.left>=r.left&&b.right<=r.right})}});assert.ok(fit.fonts.every(size=>size>=9.99),'visible settings text must retain a 10px logical font floor');assert.ok(fit.content<=fit.height,'all settings must fit; clipping is not a fix');assert.ok(fit.contentWidth<=fit.width);assert.equal(fit.overflow,'hidden');assert.ok(fit.children.every(Boolean));await shell(p).screenshot({path:`docs/evidence/ui-audit/text-settings-${width}.png`})}
