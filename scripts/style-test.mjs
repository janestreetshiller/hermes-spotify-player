...
try {
 ...
 await shell.getByRole('tab',{name:'Visualizer',exact:true}).click();await button('Enable Spotify audio analysis').waitFor()
 ...
 await button('Enable Spotify audio analysis').click();await page.locator('[data-audio-state=streaming]').waitFor();await button('Stop audio analysis').click();await page.locator('[data-audio-state=off]').waitFor()
 assert.ok((await calls('/visualizer/stop')).length>0)
 results.push('Search/playlist/lyrics remain reachable; visualizer requires consent and stops in the new shell (fixtures only)')
