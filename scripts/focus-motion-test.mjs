...
try {
 ...
 await button('Enable Spotify audio analysis').click();await page.locator('[data-audio-state=streaming]').waitFor()
 ...
 assert.ok(await button('Enable Spotify audio analysis').isDisabled())
 ...
 results.push('Native-host reduced motion stops audio, removes GPU canvas, disables animation and enable control')
