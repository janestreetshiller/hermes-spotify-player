...
async function stable(){
 return shell.evaluate(async e=>{
  ...
  return {ratio:frame.width/frame.height,fonts:[...e.querySelectorAll('.spotify-title,.spotify-meta,.spotify-time')].map(n=>parseFloat(getComputedStyle(n).fontSize)*Number(e.style.getPropertyValue('--player-scale'))),miniFit:(()=>{const n=e.querySelector('.spotify-mini-lcd');return !n?{fits:true}:{fits:n.scrollHeight<=n.clientHeight&&n.scrollWidth<=n.clientWidth,h:n.clientHeight,s:n.scrollHeight,w:n.clientWidth,sw:n.scrollWidth}})(),drift,outside:first.filter(r=>r.x<frame.x-.5||r.y<frame.y-.5||r.x+r.w>frame.right+.5||r.y+r.h>frame.bottom+.5),overlap:first.flatMap((a,i)=>first.slice(i+1).filter(b=>Math.min(a.x+a.w,b.x+b.w)-Math.max(a.x,b.x)>.5*scale&&Math.min(a.y+a.h,b.y+b.h)-Math.max(a.y,b.y)>.5*scale).map(b=>[a.label,b.label])),viewport:(()=>{const v=e.closest('.spotify-retro-viewport');return {width:v.clientWidth,scroll:v.scrollWidth}})()}
 })
}
try{
 for(const model of models){
  for(const mode of ['on','off','mini']){
   ...
   for(const width of [234,280,320,420])for(const zoom of [.8,1,1.25,1.75]){
    ...
    const g=await stable();const logicalScale=Math.min(1,g.viewport.width/320),expectedHeight=mode==='mini'?Math.max(70,88*logicalScale):({on:280,off:112}[mode])*logicalScale;assert.ok(Math.abs(g.ratio-(320*logicalScale/expectedHeight))<.001,`${model}/${mode}: specified responsive geometry (70px mini readability floor)`);assert.ok(g.fonts.every(size=>size>=9.99),`${model}/${mode}: metadata below 10px logical floor`);assert.ok(g.miniFit.fits,`${model}/${mode}/${width}/${zoom}: mini metadata clipped ${JSON.stringify(g.miniFit)}`);assert.equal(g.drift,0,`${model}/${mode}: layout wobble`);assert.deepEqual(g.outside,[],`${model}/${mode}: controls outside shell`);assert.deepEqual(g.overlap,[],`${model}/${mode}: overlapping controls`);assert.ok(g.viewport.scroll<=g.viewport.width+1,`${model}/${mode}/${width}/${zoom}: horizontal overflow ${JSON.stringify(g.viewport)}`);cases++
   }
