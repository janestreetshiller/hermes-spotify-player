...
Leaving Visual, pausing, hiding/unmounting the view, screen collapse and reduced motion release capture. Abandoned viewer leases expire after five seconds; stale visual data clears after two seconds. Missing audio or permission is displayed honestly. Hermes/macOS may retain their own ordinary operational logs; this plugin does not collect or transmit crash reports.
...
The Taste palette can prepare a copyable prompt containing the entered taste brief and seed tracks; it does not append an account's liked-song sample. Merely preparing or copying it does not call an LLM. If you submit it to an agent, its contents are governed by that agent/provider's privacy settings.
