...
Selecting **Visual** starts analysis immediately; there is no second Enable step or live status/Stop overlay. This selection is session-local: restoring a saved Visual view on a fresh mount does not authorize capture. Capture stops when leaving Visual, on pause, hidden/unmounted view, screen collapse, or reduced motion. The server expires abandoned viewer leases after five seconds and zeroes stale display data after two seconds. A dead parent also ends the helper. OS reduced motion always wins; the standalone adapter additionally honors the native host preference. No clock/random animation stands in for missing audio.
